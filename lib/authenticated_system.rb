module AuthenticatedSystem
  protected
    ...
    def current_uzytkownik
      @current_uzytkownik ||= (login_from_session || login_from_cookie) unless @current_uzytkownik == false
    end
    ...
    def self.included(base)
      base.send :helper_method, :current_uzytkownik, :logged_in?, :authorized? if base.respond_to? :helper_method
    end
    ...
    def login_from_basic_auth
      authenticate_with_http_basic do |login, password|
        self.current_uzytkownik = Uzytkownik.authenticate(login, password)
      end
    end
    ...
    # for the paranoid: we _should_ be storing uzytkownik_token = hash(cookie_token, request IP)
    ...
    def logout_keeping_session!
      ...
      @current_uzytkownik.forget_me if @current_uzytkownik.is_a? Uzytkownik
      @current_uzytkownik = false     # not logged in, and don't do it for me
      kill_remember_cookie!     # Kill client-side auth cookie
      session[:uzytkownik_id] = nil   # keeps the session but kill our variable
    ...
    def valid_remember_cookie?
      return nil unless @current_uzytkownik
      (@current_uzytkownik.remember_token?) && 
        (cookies[:auth_token] == @current_uzytkownik.remember_token)
    end
    
    # Refresh the cookie auth token if it exists, create it otherwise
    def handle_remember_cookie!(new_cookie_flag)
      return unless @current_uzytkownik
      case
      when valid_remember_cookie? then @current_uzytkownik.refresh_token # keeping same expiry date
      when new_cookie_flag        then @current_uzytkownik.remember_me 
      else                             @current_uzytkownik.forget_me
      end
    ...
    def send_remember_cookie!
      cookies[:auth_token] = {
        :value   => @current_uzytkownik.remember_token,
        :expires => @current_uzytkownik.remember_token_expires_at }
    end
