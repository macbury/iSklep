...
class UzytkownikTest < ActiveSupport::TestCase
  ...
  def test_should_initialize_activation_code_upon_creation
    uzytkownik = create_uzytkownik
    uzytkownik.reload
    assert_not_nil uzytkownik.activation_code
  end
  ...
  def test_should_not_rehash_password
    ...
    assert_equal uzytkownicy(:quentin), Uzytkownik.authenticate('quentin2', 'monkey')
  end

  def test_should_authenticate_uzytkownik
    assert_equal uzytkownicy(:quentin), Uzytkownik.authenticate('quentin', 'monkey')
  end
...
protected
  def create_uzytkownik(options = {})
    record = Uzytkownik.new({ :login => 'quire', :email => 'quire@example.com', :password => 'quire69', :password_confirmation => 'quire69' }.merge(options))
    record.save
    record
  end
end
